...
contract('Bridge - [create a deposit proposal (voteProposal) with relayerThreshold = 1]', async (accounts) => {
    ...
    beforeEach(async () => {
        ...
        await Promise.all([
            ...
            await upgrades.deployProxy(await ethers.getContractFactory("BridgeUpgradeable"), originDomainID, [originChainRelayerAddress], relayerThreshold, 0, 100, ETHresourceID, weth.address).then(instance => BridgeInstance = instance)
        ]);
    ...
    it('originChainRelayerAddress should be marked as voted for proposal', async () => {
        ...
        const hasVoted = await BridgeInstance._hasVotedOnProposal(
...
contract('Bridge - [create a deposit proposal (voteProposal) with relayerThreshold > 1]', async (accounts) => {
    ...
    beforeEach(async () => {
        weth = await WETH.new();
        ETHresourceID = Helpers.createResourceID(weth.address, originDomainID);
        await Promise.all([
            ERC20MintableContract.new("token", "TOK").then(instance => DestinationERC20MintableInstance = instance),
            await upgrades.deployProxy(await ethers.getContractFactory("BridgeUpgradeable"), originDomainID, [originChainRelayerAddress], relayerThreshold, 0, 100, ETHresourceID, weth.address).then(instance => BridgeInstance = instance)
        ]);
