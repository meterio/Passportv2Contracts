...
contract('Bridge - [deposit - ERC1155]', async (accounts) => {
    ...
    beforeEach(async () => {
        ...
        await Promise.all([
            ...
            BridgeInstance = await upgrades.deployProxy(await ethers.getContractFactory("BridgeUpgradeable"), originDomainID, [], relayerThreshold, 0, 100, ETHresourceID, weth.address)
        ]);
        ...
        await Promise.all([
            ...
            OriginERC1155MintableInstance.mintBatch(depositerAddress, [originChainTokenID], [originChainInitialTokenAmount], Ethers.constants.HashZero)
        ]);
    ...
    it('_depositCounts should be increments from 0 to 1', async () => {
        ...
        const depositCount = await BridgeInstance._depositCounts(destinationDomainID);
