...
contract('ERC20Handler - [setResourceIDAndContractAddress]', async () => {
    ...
    beforeEach(async () => {
        ...
        BridgeInstance = await upgrades.deployProxy(await ethers.getContractFactory("BridgeUpgradeable"), domainID, [], relayerThreshold, 0, 100, ETHresourceID, weth.address);
    ...
    it("[sanity] ERC20MintableInstance1's resourceID and contract address should be set correctly", async () => {
        const retrievedTokenAddress = await ERC20HandlerInstance._resourceIDToTokenContractAddress(initialResourceIDs[0]);
        assert.strictEqual(Ethers.utils.getAddress(ERC20MintableInstance1.address), retrievedTokenAddress);

        const retrievedResourceID = await ERC20HandlerInstance._tokenContractAddressToResourceID(ERC20MintableInstance1.address);
    ...
    it('new resourceID and corresponding contract address should be set correctly', async () => {
        ...
        const retrievedTokenAddress = await ERC20HandlerInstance._resourceIDToTokenContractAddress(secondERC20ResourceID);
        assert.strictEqual(Ethers.utils.getAddress(ERC20MintableInstance2.address).toLowerCase(), retrievedTokenAddress.toLowerCase());

        const retrievedResourceID = await ERC20HandlerInstance._tokenContractAddressToResourceID(ERC20MintableInstance2.address);
    ...
    it('existing resourceID should be updated correctly with new handler address', async () => {
        ...
        const bridgeHandlerAddress = await BridgeInstance._resourceIDToHandlerAddress(initialResourceIDs[0]);
    ...
    it('existing resourceID should be replaced by new resourceID in handler', async () => {
        ...
        const retrievedContractAddress = await ERC20HandlerInstance._resourceIDToTokenContractAddress(secondERC20ResourceID);
