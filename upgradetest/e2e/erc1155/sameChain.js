...
contract('E2E ERC1155 - Same Chain', async accounts => {
    ...
    beforeEach(async () => {
        ...
        await Promise.all([
            await upgrades.deployProxy(await ethers.getContractFactory("BridgeUpgradeable"), domainID, [relayer1Address, relayer2Address], relayerThreshold, 0, 100, ETHresourceID, weth.address).then(instance => BridgeInstance = instance),
        ...
        await Promise.all([
            ERC1155MintableInstance.mintBatch(depositerAddress, [tokenID], [initialTokenAmount], Ethers.constants.HashZero),
    ...
    it("Should withdraw funds", async () => {
        ...
        await ERC1155MintableInstance.safeTransferFrom(depositerAddress, ERC1155HandlerInstance.address, tokenID, depositAmount, Ethers.constants.HashZero, { from: depositerAddress });
