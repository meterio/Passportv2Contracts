...
contract('Gas Benchmark - [Execute Proposal]', async (accounts) => {
    ...
    const deposit = (resourceID, depositData) => BridgeInstance.deposit(domainID, resourceID, depositData, { from: depositorAddress });
    ...
    before(async () => {
        ...
        await Promise.all([
            BridgeContract.new(domainID, initialRelayers, relayerThreshold, 0, 100, ETHresourceID, weth.address).then(instance => BridgeInstance = instance),
