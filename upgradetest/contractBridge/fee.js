...
contract('Bridge - [fee]', async (accounts) => {
    ...
    beforeEach(async () => {
        ...
        await Promise.all([
            ...
            BridgeInstance = await upgrades.deployProxy(await ethers.getContractFactory("BridgeUpgradeable"), originDomainID, [relayer], 0, 0, 100, ETHresourceID, weth.address).then(instance => BridgeInstance = instance)
        ]);
    ...
    it('deposit passes if valid amount supplied', async () => {
        ...
        assert.equal(web3.utils.fromWei((await BridgeInstance._fee()).toString(),"ether"), "0.5");
    ...
    it('distribute fees', async () => {
        ...
        assert.equal(web3.utils.fromWei((await BridgeInstance._fee()).toString(),"ether"), "1");

        // check the balance is 0
        assert.equal(web3.utils.fromWei((await web3.eth.getBalance(BridgeInstance.address)).toString(),"ether"), "0");
        await BridgeInstance.deposit(destinationDomainID, resourceID, depositData, {value: Ethers.utils.parseEther("1")})
        assert.equal(web3.utils.fromWei((await web3.eth.getBalance(BridgeInstance.address)).toString(),"ether"), "1");
