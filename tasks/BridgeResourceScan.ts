...
import { Bridge, ERC20Handler, TokenERC20 } from "../typechain-types";
...
const parseHandleResouceId = async (
  ...
  if (handler_address == "0x0000000000000000000000000000000000000000") {
    console.log(
      "###### resouceId: ",
      resourceId,
      "Could not find the resourceId ..."
    );
    return;
  }
  const handler_contract = (await hre.ethers.getContractAt(
    "ERC20Handler",
    handler_address
  )) as ERC20Handler;

  const erc20_address =
    await handler_contract._resourceIDToTokenContractAddress(resourceId);
  ...
  if (!isNative) {
    const erc20_contract = (await hre.ethers.getContractAt(
      "TokenERC20",
      erc20_address
    )) as TokenERC20;
