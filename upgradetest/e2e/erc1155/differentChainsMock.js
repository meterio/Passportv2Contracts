...
contract('E2E ERC1155 - Two EVM Chains', async accounts => {
    ...
    beforeEach(async () => {
        ...
        const originETHresourceID = Helpers.createResourceID(weth.address, originDomainID);
        const destinationETHresourceID = Helpers.createResourceID(weth.address, destinationDomainID);
        await Promise.all([
            await upgrades.deployProxy(await ethers.getContractFactory("BridgeUpgradeable"), originDomainID, [originRelayer1Address, originRelayer2Address], originRelayerThreshold, 0, 100, originETHresourceID, weth.address).then(instance => OriginBridgeInstance = instance),
            await upgrades.deployProxy(await ethers.getContractFactory("BridgeUpgradeable"), destinationDomainID, [destinationRelayer1Address, destinationRelayer2Address], destinationRelayerThreshold, 0, 100, destinationETHresourceID, weth.address).then(instance => DestinationBridgeInstance = instance),
        ...
        await OriginERC1155MintableInstance.mintBatch(depositerAddress, [tokenID], [initialTokenAmount], Ethers.constants.HashZero);
