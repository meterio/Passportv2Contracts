...
contract('ERC1155Handler - [Deposit Burn ERC1155]', async (accounts) => {
    ...
    beforeEach(async () => {
        ...
        await Promise.all([
            ...
            ERC1155MintableInstance1.mintBatch(depositerAddress, [tokenID], [tokenAmount], Ethers.constants.HashZero)
        ]);
    ...
    it('[sanity] burnableContractAddresses should be marked true in _burnList', async () => {
        for (const burnableAddress of burnableContractAddresses) {
            const isBurnable = await ERC1155HandlerInstance._burnList(burnableAddress);
