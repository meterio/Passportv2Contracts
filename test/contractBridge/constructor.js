...
contract('Bridge - [constructor]', async accounts => {
    ...
    it('Bridge should not allow to set initialRelayerThreshold above 255', async () => {
        return TruffleAssert.fails(BridgeContract.new(domainID, initialRelayers, 256, 0, 100, ETHresourceID, weth.address), "value does not fit in 8 bits");
    });
    ...
    it('Bridge should not allow to set expiry above 2**40 - 1', async () => {
        return TruffleAssert.fails(BridgeContract.new(domainID, initialRelayers, initialRelayerThreshold, 0, BN(2).pow(BN(40)), ETHresourceID, weth.address), "value does not fit in 40 bits");
    });
});
