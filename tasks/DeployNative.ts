...
task("deploy-native", "deploy and regist native")
  ...
  .setAction(async ({ from, name, symbol }, { ethers, run, network }) => {
    await run("compile");
    ...
    if (Number(from) == config.id) {
      let handlerAddress = await bridgeInstant._resourceIDToHandlerAddress(
        resid
      );
      ...
      let hasRole = await tokenInstant.hasRole(
        MINTER_ROLE,
        config.erc20Handler
      );
      if (!hasRole) {
